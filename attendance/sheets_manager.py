...
def get_current_month_sheet(client, creds):
    ...
    try:
        ...
        print(f"🔧 Attempting to share sheet with {YOUR_EMAIL}")
        drive_service.permissions().create(
            fileId=sheet.id,
            body=permission,
            sendNotificationEmail=True
        ).execute()
        #print(f"✅ Sheet shared successfully. Permission ID: {response.get('id')}")
        print(f"🔗 Sheet link: https://docs.google.com/spreadsheets/d/{sheet.id}")
        print(f"📤 Sheet shared with {YOUR_EMAIL}")
    except Exception as e:
        print("❌ ERROR while sharing the sheet")
        print(f"⚠️ Failed to share with {YOUR_EMAIL}: {e}")
        import traceback
        traceback.print_exc()
